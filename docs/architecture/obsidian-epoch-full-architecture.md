...
identity.issue
  ...
  -> journey.status/briefing final verification rebuilds the discarded-mirror result page and conditional warnings
...
| Agent-native Journey | `game-core/journey`, MCP Skill, result projections | `hosted_action_recorded`, terminal resource/item/`attribute_gained` effects, optional `journey_world_solidified`; final `未及格` records a discarded mirror and does not archive identity | mission/task plan, signed SceneContracts, `storyReport` v3, `evaluation.rewardConversion`, conditional identity warning, immutable verification page | Alpha 1 |
