...
const acceptanceScript = path.join(scriptDirectory, "phase6-ten-run.mjs");
...
function secretPathsInValue(value, pathPrefix, paths) {
...
function secretFindingForStdoutLine(line) {
  const reasons = [];
  const paths = [];
...
function cleanupEvidenceFile(outputPath, removeNewFile) {
  try {
    if (removeNewFile) {
...
function runExternalCommand(argv, outputPath, timeoutMs) {
  return new Promise((resolve) => {
    ...
    function terminateForStdoutSecret(finding) {
    ...
    function writeCheckedLine(line) {
...
function runAcceptance(outputPath, experimentId) {
  return new Promise((resolve) => {
...
function writeRunnerFailure(message) {
