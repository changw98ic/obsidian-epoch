...
import { phase6AuthoritativeRagTrace } from "./phase6RagTestFixtures.ts";

const repoRoot = resolve(dirname(fileURLToPath(import.meta.url)), "../../..");
const evidencePath = resolve(repoRoot, "tools/agent-server/phase6-ten-run-evidence.ts");
const scenarioProtocol = [
  ["low-prepared-resource", "resource_acquisition"],
  ["low-underprepared-information", "information_acquisition"],
  ["medium-prepared-structured", "structured_challenge"],
  ["medium-borderline-companion", "companion_support"],
  ["medium-mismatched-preserve", "resource_preservation"],
  ["high-prepared-priority", "priority_commission"],
  ["high-underprepared-crisis", "crisis_retreat"],
...
test("Phase 6 ten-run evidence derives golden gate inputs from sanitized tool outputs", () => {
  withTempRepoDir((root) => {
    ...
    assertGatePass("phase6-player-panel-gate.ts", [
    ...
    assertGatePass("phase6-economy-gate.ts", ["--expected-runs", "10", "--input", repoPath(join(outputDir, "economy-audit.jsonl"))]);
    assertGatePass("phase6-progression-gate.ts", ["--input", repoPath(join(outputDir, "progression.jsonl"))]);
    assert.match(readFileSync(join(outputDir, "receipts.jsonl"), "utf8").split("\n")[0], /ragDelta/);
    assertGatePass("phase6-rag-gate.ts", [
      "--query-evaluations", repoPath(join(outputDir, "rag-query-evaluations.jsonl")),
      "--run-receipts", repoPath(join(outputDir, "receipts.jsonl")),
    ]);
    assertGatePass("phase6-world-gate.ts", ["--input", repoPath(join(outputDir, "world-evidence.jsonl"))]);
    assertGatePass("phase6-score-gate.ts", ["--expected-runs", "10", repoPath(join(outputDir, "scores.jsonl"))]);
    assertGatePass("phase6-shop-gate.ts", ["--input", repoPath(join(outputDir, "shop.jsonl"))]);
    assertGatePass("phase6-crafting-gate.ts", [
