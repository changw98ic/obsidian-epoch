...
import { canonicalJson, manifestHash } from "../phase6-evidence-pack.ts";
import { buildReport } from "../phase6-release-gate.ts";
import { ATTESTATION_KEY_ENV, receiptHash } from "../phase6-command-runner.ts";
...
function runEvidencePack(args, keyFile) {
  return spawnSync(process.execPath, ["tools/agent-server/phase6-evidence-pack.ts", ...args], {
    cwd: repoRoot,
    encoding: "utf8",
    shell: false,
...
function runReleaseGate(configPath, keyFile) {
  return spawnSync(process.execPath, ["tools/agent-server/phase6-release-gate.ts", "--config", configPath], {
    cwd: repoRoot,
    encoding: "utf8",
    shell: false,
    env: commandEnvironment(keyFile),
