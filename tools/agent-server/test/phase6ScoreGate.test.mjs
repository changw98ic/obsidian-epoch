...
function runGate(values) {
  const directory = mkdtempSync(join(process.cwd(), "tools/agent-server/test/.phase6-score-gate-"));
  ...
  const result = spawnSync(process.execPath, ["tools/agent-server/phase6-score-gate.mjs", "--expected-runs", "10", input], {
    cwd: process.cwd(),
    encoding: "utf8",
  });
