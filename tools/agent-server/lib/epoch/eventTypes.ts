/**
 * Pure type definitions for the Epoch event system.
 *
 * Extracted from events.ts to break circular dependencies. This module
 * contains ONLY type definitions (interfaces, type aliases, mapped types)
 * and compile-time constants used in `typeof` positions. It has zero
 * runtime code, which means TypeScript's `import type` erasure ensures
 * the bundler never needs to resolve runtime module loading order for
 * type-only imports through this file.
 *
 * All runtime functions (createEpochEvent, assertEpochEvent) remain in
 * events.ts, which re-exports every type defined here so that downstream
 * consumers can continue importing from events.ts without changes.
 */
...
import type { CausalWorldEventRecordedPayload } from "./causalEpochAdapter.ts";
...
export interface JourneyWorldSolidifiedPayload {
   ...
   * produce a canonical event with a different position (and identity doubt
   * entries may share one viability event in future policy versions), so the
   * mapping is authoritative and never reconstructed positionally.
   */
 ...
 *
 * Anti-loop invariant (spec section 6.8): the lifetime delta implied by this
